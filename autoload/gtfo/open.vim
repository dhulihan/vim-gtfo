...
func! gtfo#open#term(dir, cmd) abort "{{{
  ...
  if s:istmux
    if s:tmux_1_6
      silent call system('tmux split-window -v -p 10 \; send-keys "cd ''' . l:dir . ''' && clear" C-m')
    else
      " TODO: use a global var for this
      silent call system("tmux split-window -v -p 10 -c '" . l:dir . "'")
    endif
  elseif s:iskitty
  ...
  elseif s:iswin
    ...
    elseif s:termpath =~? 'wt' && executable('wt')
      silent exe '!start '.s:termpath.' -d '.shellescape(l:dir,1)
